-- =====================================================
-- FIX UUID TYPE MISMATCH ERROR IN INVENTORY DEDUCTION
-- =====================================================
-- This script creates a new version of the deduct_inventory_with_validation function
-- that properly handles UUID type casting to fix the "operator does not exist: uuid = text" error

-- Drop the old function if it exists
DROP FUNCTION IF EXISTS public.deduct_inventory_with_validation_v2(TEXT, TEXT, INTEGER, TEXT, TEXT, TEXT, TEXT);

-- Create the new function with proper UUID type casting
CREATE OR REPLACE FUNCTION public.deduct_inventory_with_validation_v2(
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE id = warehouse_uuid AND is_active = true
    ) THEN
    ...
    -- Get current inventory quantity and minimum stock
    SELECT
    ...
    RAISE NOTICE '📊 الكمية الحالية: %, الكمية المطلوبة: %', current_quantity, p_quantity;
...
GRANT EXECUTE ON FUNCTION public.deduct_inventory_with_validation_v2(TEXT, TEXT, INTEGER, TEXT, TEXT, TEXT, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.deduct_inventory_with_validation_v2(TEXT, TEXT, INTEGER, TEXT, TEXT, TEXT, TEXT) TO service_role;
