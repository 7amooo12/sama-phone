...
class ImageCluster {
  ...
  factory ImageCluster.fromJson(Map<String, dynamic> json) {
    return ImageCluster(
      ...
      centroid: List<double>.from(json['centroid']),
    );
  }
}
    ...
    try {
      ...
      while (!hasConverged && iterations < maxIterations) {
        ...
        productVectors.forEach((productId, vector) {
          int nearestCentroidIndex = _findNearestCentroidIndex(vector, centroids);
        ...
        for (int i = 0; i < k; i++) {
          if (assignments[i]!.isEmpty) {
            ...
            List<double> sum = List<double>.filled(vectorLength, 0.0);
